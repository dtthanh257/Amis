<template>
  <div class="container">
    <div class="container-employ">
      ...
      <MisaButton :buttonClass="`btn btn-pri`" :buttonText="'Thêm mới nhân viên'" @click="showEmployeeForm()" />
    </div>
    <div class="container-content">
      <div class="container-content-action">
        <div class="checked-emp d-flex-row gap-16 align-item-center" style="flex: 1">
          <div class="checked-row" v-if="this.checkedRow >= 1">
            Đã chọn: {{ this.checkedRow }}
          </div>
          <div class="check-remove" style="color: red; cursor: pointer" v-if="this.checkedRow >= 1"
            @click="deselectAll">
            Bỏ chọn
          </div>
          <button class="btn btn-sec" style="height: 36px; width: fit-content" v-if="this.checkedRow >= 1"
        ...
        <div class="textfield default">
          <div class="textfield-with-acion">
            <input type="text" name="input" placeholder="Tìm theo mã, tên nhân viên" v-model="filterText"
              @keyup.enter="getFilterEmployee(filterText)" />
            <span @click="getFilterEmployee(filterText)"><i class="fa-solid fa-magnifying-glass"></i></span>
          </div>
        </div>
          ...
          <tbody id="employeeData">
            <tr v-for="(employee, index) in this.tableData" :key="index"
              ...
              <td class="text-align-center d-flex-row-center" style="position: relative">
                <input type="checkbox" @click="toggleCheckbox(employee)" v-model="employee.isChecked" />
              </td>
              ...
              <td class="text-align-right">
                <div class="d-flex-row-center gap-8" style="position: relative">
                  <button class="btn btn-fix btn-link" @click="
                  ...
                  <button class="btn-link btn-drop-down-fix show-fix-menu"
                    @click="showFixMenu(employee.EmployeeId)"></button>
                  <ContextMenu :idEmployee="employee.EmployeeId" v-if="employeeIdContextMenu == employee.EmployeeId"
                    @employeeDeleted="getEmployeesData"></ContextMenu>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div class="employ-form-container">
        <div class="form-header">
          ...
          <div class="form-header-action">
            ...
            <button class="form-close" id="formClose" @click="closeForm()"><span>Đóng form</span></button>
          </div>
        </div>
        <div class="d-flex-row gap-28">
          <div class="employ-info w1-2">
            <div class="d-flex-row gap-8">
              <div class="textfield w2-5" :class="{
                ...
                <input id="txtEpCode" v-model="this.formUpdate.employeeCode" type="text" name="input" placeholder=""
                  ref="inputElement" />
              </div>
              <div class="textfield w3-5" :class="{
                'textfield-error textfield-name-error': textfieldName,
              }" id="nameTextfield">
                <label>Tên <span class="require">*</span></label>
                <input id="txtEmployeeName" v-model="this.formUpdate.fullName" type="text" name="input"
                  placeholder="" />
              </div>
            </div>
            ...
            <div class="textfield" :class="{
              ...
              <input id="txtEmployeeDepartment" v-model="this.formUpdate.departmentName" @change="inputOnchange()"
                type="text" name="input" placeholder="" />
            </div>
            <div class="textfield">
              <label>Chức danh</label>
              <input id="txtEmployeePosition" type="text" v-model="this.formUpdate.positionName" name="input"
                placeholder="" />
            </div>
          </div>
          <div class="employ-info w1-2">
            <div class="d-flex-row gap-16">
              <div class="textfield">
                ...
                <input id="employeeDateOfBirth" v-model="this.formUpdate.dateOfBirth" type="date" name="input"
              ...
              <div class="textfield">
                ...
                <div class="textfield-flex" style="accent-color: var(--color-accent);">
                  <div class="gender">
                    <input style="cursor: pointer" type="radio" name="gender" value="0" placeholder="" checked />Nam
                  </div>
                  <div class="gender">
                    <input style="cursor: pointer" type="radio" name="gender" value="1" placeholder="" />Nữ
                  </div>
                  <div class="gender">
                    <input style="cursor: pointer" type="radio" name="gender" value="2" placeholder="" />Khác
                  </div>
                </div>
              </div>
            </div>
            <div class="d-flex-row">
              <div class="textfield-flex gap-8">
                <div class="textfield w3-5 tooltip-place-id">
                  <label>Số CMND</label>
                  <input id="employeeIdentity" type="number" name="input" v-model="this.formUpdate.identityNumber"
                    placeholder="" />
                ...
                <div class="textfield w2-5">
                  ...
                  <input id="employeeIdentityDate" v-model="this.formUpdate.identityDate" type="date" name="input"
                    placeholder="DD/MM/YYYY" />
                </div>
              </div>
            </div>
            <div class="textfield">
              <label>Nơi cấp</label>
              <input id="employeeIdentityPlace" type="text" v-model="this.formUpdate.indentityPlace" name="input"
                placeholder="" />
            </div>
          </div>
        </div>
        <div class="d-flex-column">
          <div class="textfield w1-1">
            <label>Địa chỉ</label>
            <input id="employeeAddress" type="text" name="input" v-model="this.formUpdate.address" placeholder="" />
          </div>
          <div class="d-flex-row gap-8 w4-5">
            <div class="textfield w1-1 tooltip-place-phone">
              <label>ĐT di động</label>
              <input id="employeePhone" type="number" name="input" v-model="this.formUpdate.phoneNumber"
                placeholder="" />
            ...
            <div class="textfield w1-1">
              <label>Email</label>
              <input id="employeeEmail" v-model="this.formUpdate.email" type="text" name="input" placeholder="" />
            </div>
          </div>
          <div class="d-flex-row gap-8 w4-5">
            <div class="textfield w1-1">
        ...
        <div class="form-footer">
          <div class="d-flex-row gap-8">
            ...
            <div class="form-footer-action gap-12">
              <button class="btn btn-sec" v-if="formMode === $MISAEnum.FORM_MODE.ADD" @click="addAndCloseForm()">
                Cất
              </button>
              <button class="btn btn-sec" v-if="formMode === $MISAEnum.FORM_MODE.UPDATE" @click="updateEmployees()">
                Cất
              </button>
              <button class="btn btn-pri" :dialogText="dialogText" :dialogTitle="dialogTitle" :dialog="dialog"
                @click="validateText()">
                Cất và thêm
              </button>
            </div>
    ...
    <TheDialog v-if="dialogName" @close-dialog="closeDialog()" :dialogTitle="'Lưu ý'" :errors="errors"
      :dialogText="'Tên nhân viên không được phép để trống.'" />

    <TheDialog v-if="dialogHaveCode" @close-dialog="closeDialog()" :dialogTitle="'Cảnh báo'" :errors="errors"
      :dialogText="'Mã nhân viên đã tồn tại trong hệ thống'" :dialogHaveCode="dialogHaveCode" />
    <TheDialog v-if="dialogFailInput" @close-dialog="closeDialog()" :dialogTitle="'Cảnh báo'" :errors="errors"
      :dialogText="'Email sai định dạng'" :dialogHaveCode="dialogFailInput" />
...
import MisaButton from "../base/TheButton.vue";
